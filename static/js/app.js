...
async function preview(){
  ...
  // Privacy: previews are generated by our own /api/preview only.
  // No user input is ever sent to a third party. On failure we retry once
  // internally, then fail with a clear error and keep the last good image.
  const payload = {
    content,
    type: state.type,
    fg_color: state.fg,
  ...
  for(let attempt = 0; attempt < 2; attempt++){
    try{
      ...
      if(!res.ok){
        lastErr = new Error(`Preview failed (${res.status})`);
        continue; // retry once
      }
      ...
      lastErr = new Error(j.error || "Preview failed");
    }catch(e){
  ...
  if(loading) loading.style.display="none";
  if(frame) frame.classList.remove('loading');
  toast("Preview failed — check connection and try again. No data was sent to any third party.", true);
}
